<?php
// Captura qualquer output (avisos/notices PHP) antes do JSON para não quebrar o parse
ob_start();
// Suprime warnings de display em produção (não no log)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    if ($_POST['username'] === $admin_user && password_verify($_POST['password'], $admin_hash)) {
...
function processarUpload($file_entry, $prefix, $upload_dir, $allow_video = true) {
    ...
    // Calcular caminho relativo a partir do __DIR__
    $rel = ltrim(str_replace(__DIR__, '', $fullpath), '/\\');
    return $rel; // ex: "uploads/clientes/equatorial/cli_123_456.jpg"
}
            ...
            if ($rel) {
                foreach ($salas_data['salas'] as &$sala) {
                    if ($sala['id'] == $sala_id) {
                        ...
                        $sala['fotos'][] = $rel;
                        if (empty($sala['imagem_principal']) || strpos($sala['imagem_principal'], 'fotos_predio/') === 0) {
        ...
        foreach ($salas_data['salas'] as &$sala) {
            if ($sala['id'] == $sala_id) {
                ...
                if (($sala['imagem_principal'] ?? '') === $foto_path) {
                    $sala['imagem_principal'] = !empty($sala['fotos']) ? $sala['fotos'][0] : 'fotos_predio/foto_predio_1.jpg';
                }
                break;
            }
        }
...
if ($is_logged_in && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_room') {
        ...
        $salas_data['salas'][] = [
            ...
            'imagem_principal'=> 'fotos_predio/foto_predio_1.jpg',
            'fotos'           => ['fotos_predio/foto_predio_1.jpg'],
    ...
    if ($action === 'change_credentials') {
        ...
        if (!password_verify($cur_pass, $admin_hash)) {
            ...
            $message = '<div class="msg-error">❌ Senhas não coincidem.</div>';
        } elseif (!empty($new_pass) && strlen($new_pass) < 4) {
            $message = '<div class="msg-error">❌ Senha mínimo 4 caracteres.</div>';
        } else {
            $up = ['user'=>!empty($new_user)?$new_user:$admin_user,'hash'=>!empty($new_pass)?password_hash($new_pass,PASSWORD_BCRYPT):$admin_hash];
            file_put_contents($cred_file, json_encode($up)); @chmod($cred_file, 0600);
            $message = '<div class="msg-success">✅ Credenciais atualizadas!</div>';
            $admin_user = $up['user']; $admin_hash = $up['hash'];
        }
    }
}
...
<div class="login-wrap">
    <div class="login-box">
        ...
        <?php if (isset($login_error)): ?><div class="login-error"><?= htmlspecialchars($login_error) ?></div><?php endif; ?>
        <form method="POST">
...
<div class="admin-wrap">
    ...
    <div id="tab-video" class="tab-panel <?= $tab_ativa==='video'?'active':'' ?>">
        <div class="card-section">
            <h3>🎬 Vídeo da Página Inicial</h3>
            <p class="desc">Este vídeo aparece como fundo na seção principal (hero) do site. Você pode adicionar ou remover vídeos.</p>

            <?php
                $galeria_videos = $galeria_data['videos'] ?? (isset($galeria_data['video']) && $galeria_data['video'] ? [$galeria_data['video']] : ['videos/video predio.mp4']);
            ?>
            <div id="videos-home-list" style="margin-bottom:16px">
                <?php foreach ($galeria_videos as $vid): ?>
                <div class="home-photo-card" style="position:relative;margin-bottom:10px">
                    <video src="<?= htmlspecialchars($vid) ?>" muted preload="metadata" playsinline
                        style="width:100%;max-height:120px;object-fit:cover;display:block;background:#000;border-radius:6px"></video>
                    <p style="font-size:11px;color:#888;margin:4px 0"><?= htmlspecialchars($vid) ?></p>
                    <button type="button" class="btn-remove-home"
                        onclick="removerVideoHome('<?= addslashes($vid) ?>', this)">✕ Remover</button>
                </div>
                <?php endforeach; ?>
                <?php if (empty($galeria_videos)): ?>
                <p style="color:#999;font-size:13px">Nenhum vídeo configurado.</p>
                <?php endif; ?>
            </div>

            <div class="upload-zone" id="uz-video-home">
                <input type="file" name="video" accept="video/mp4,video/quicktime,video/webm"
                    onchange="uploadVideoHome(this)">
                <div class="upload-zone-label">
                    <div class="upload-zone-icon">🎥</div>
                    <div class="upload-zone-text">Adicionar Vídeo</div>
                    <div class="upload-zone-sub">MP4, MOV, WEBM — até <?= ini_get('upload_max_filesize') ?></div>
                </div>
            </div>
            <div class="upload-progress-wrap" id="prog-wrap-video-home">
                <div class="upload-progress-bar" id="prog-video-home"></div>
            </div>
            <div class="upload-status" id="upload-status-video-home"></div>
        </div>
    </div>
...
function setMain(salaId, path, btn) {
    fetch('admin.php?ajax=set_main_sala', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({sala_id: salaId, foto_path: path})
    })
...
function removerFoto(salaId, path, btn) {
    ...
    fetch('admin.php?ajax=remove_foto_sala', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({sala_id: salaId, foto_path: path})
    })
...
function removerFotoHome(path, btn) {
    ...
    fetch('admin.php?ajax=remove_foto_home', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({foto_path: path})
    })
    .then(function(r){ return r.json(); })
    .then(function(r) {
        if (r.ok) {
...
function editarNomePonto(id, nome) {
    ...
    if (!n || !n.trim()) return;
    var fd = new FormData();
    fd.append('id', id); fd.append('nome', n.trim());
...
function removerFotoPonto(id, foto, btn) {
    if (!confirm('Remover esta foto?')) return;
    fetch('admin.php?ajax=delete_foto_ponto', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id,foto:foto})})
        .then(function(r){return r.json();})
...
function salvarNovoPonto() {
    ...
    var fd=new FormData(); fd.append('id',0); fd.append('nome',nome);
    fetch('admin.php?ajax=save_ponto',{method:'POST',body:fd})
        .then(function(r){return r.json();})
...
function deletarPonto(id) {
    ...
    fetch('admin.php?ajax=delete_ponto',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})})
        .then(function(r){return r.json();})
...
function removerFotoCli(id, foto, btn) {
    if (!confirm('Remover esta foto?')) return;
    fetch('admin.php?ajax=delete_foto_cliente',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id,foto:foto})})
        .then(function(r){return r.json();})
        .then(function(r){if(r.ok) btn.closest('.foto-thumb').remove();});
}
...
function salvarNovoCliente() {
    ...
    if(!nome){stat.textContent='⚠️ Digite o nome';return;}
    stat.textContent='Criando...';
    var fd=new FormData(); fd.append('id',0); fd.append('nome',nome); fd.append('ativo','1');
...
function deletarCliente(id) {
    ...
    fetch('admin.php?ajax=delete_cliente',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})})
        .then(function(r){return r.json();})
        .then(function(r){if(r.ok) location.reload();});
}
function toggleCliente(id) {
    fetch('admin.php?ajax=toggle_cliente',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})})
        .then(function(r){return r.json();})
        .then(function(r){if(r.ok) location.reload();});
}

/* ═══════ VÍDEO HOME ═══════ */
function uploadVideoHome(input) {
    var file=input.files[0]; if(!file) return;
    var prog=document.getElementById('prog-video-home');
    var wrap=document.getElementById('prog-wrap-video-home');
    var stat=document.getElementById('upload-status-video-home');
    wrap.style.display='block';
    stat.textContent='Enviando '+(file.size/1048576).toFixed(1)+'MB...';
    var fd=new FormData(); fd.append('video',file);
    var xhr=new XMLHttpRequest();
    xhr.open('POST','admin.php?ajax=upload_video_home');
    xhr.upload.onprogress=function(e){if(e.lengthComputable)prog.style.width=Math.round(e.loaded/e.total*99)+'%';};
    xhr.onload=function(){
        try{var r=JSON.parse(xhr.responseText);stat.textContent=r.msg||(r.ok?'✅ Enviado!':'❌ Erro');if(r.ok)setTimeout(function(){location.reload();},1000);}catch(e){stat.textContent='❌ Erro';}
        wrap.style.display='none';
    };
    xhr.onerror=function(){stat.textContent='❌ Falha de rede';wrap.style.display='none';};
    xhr.send(fd); input.value='';
}
function removerVideoHome(src, btn) {
    if(!confirm('Remover este vídeo?')) return;
    fetch('admin.php?ajax=remove_video_home',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({video_path:src})})
        .then(function(r){return r.json();})
        .then(function(r){if(r.ok){btn.closest('.home-photo-card').remove();showToast('✅ Vídeo removido!',true);}});
}
</script>
</body>
</html>
